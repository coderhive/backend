    ...
    async getCollectionByIdentifierSpecial(identifier, idType, specialValue) {
        try {
            let entity = await this._db(this._entityName)
                ...
                .select(`${this._entityName}.id`, `${this._entityName}.user_id`, 'User.display_name', `${this._entityName}.component_id`, 'Component.title', 'Component.description', `${this._entityName}.created_at`, `${this._entityName}.updated_at`, `${this._entityName}.${specialValue}`)
            console.log(entity);
            return entity;
        } catch (error) {
            throw error
