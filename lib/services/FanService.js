...
class FanService {
    ...
    async getByIdentifier(args, parent) {
        try {
            ...
            return await this.fanRepository.getCollectionByIdentifier(componentId, "component_id");
        } catch (error) {
            console.log(error)
        }
    }
    ...
    async getByUserId(args, parent) {
        try {
            // TODO Authenticate for admin or ownership
            let componentId = parent.id;
            return await this.fanRepository.getCollectionByIdentifier(componentId, "user_id");
        } catch (error) {
            console.log(error)
        }
    }
